...
class StochHECRAS:
    ...
    def launch_sims(self):
        ...
        for j in range(self.stochICE.NSims):
            ...
            tif_filename = os.path.join(
                ...
                f"WSE_{self.flow}_{self.ice_thickness}_{self.phi}.tif"
            )
    ...
    def randomize_variables(self):
        ...
        # Randomly generate ice thickness and friction angle (phi)
    ...
    def get_random_flowrate(self):
        """Generates a random flow rate within the specified range and updates the flow file."""
        secure_random = random.SystemRandom()
...
#     def set_ice_jam_location(self):
#         """
#         Updates ice jam location parameters only for cross-sections where the reach 
#         matches self.stochICE.ice_jam_reach, and where the chainage is within the specified range.
...
#         """
#         for item in self.xs_data_modified:
#             # Check if the Reach matches the desired ice jam reach and chainage is within the location range
#             if (self.xs_data_modified[item]['Reach'] == self.stochICE.ice_jam_reach and
#                 self.xs_data_modified[item]['chainage'] >= self.location[0] and
#                 self.xs_data_modified[item]['chainage'] <= self.location[1]):
                
#                 # Set ice jam location values
#                 self.xs_data_modified[item]["Ice Is Channel"]['val'] = str(-1)
#                 self.xs_data_modified[item]["Ice Is OB"]['val'] = str(-1)
...
#         """
#         IMPORTANT: This code only works for a single reach, or an upstream reach
#         that divides into two branches downstream (e.g. the Chateauguay River).
#         Handling different flow files, for example two rivers joining into one, then
#         more logic will need to be introduced. 
#         """
...
#         # Initialize a list to store the updated lines
#         updated_lines = []
        
#         # Initialize variables to track river entries and reach values
#         river_reaches = []
#         current_reach_index = -1
        
#         # Iterate over the lines to identify the "River Rch & RM" entries and their following values
#         for i, line in enumerate(lines):
#             if "River Rch & RM" in line:
#                 current_reach_index += 1
#                 river_reaches.append(i)  # Track the line numbers with river entries
#             updated_lines.append(line)  # Keep adding lines to the new list
        
#         # Update the values for each "River Rch & RM" entry
#         for idx, reach_line in enumerate(river_reaches):
#             if idx == 0:
#                 # Update the value below the first "River Rch & RM" with the provided river_1_value
#                 updated_lines[reach_line + 1] = f"     {self.flow}\n"
#             else:
#                 # Update the value below subsequent "River Rch & RM" entries with half of river_1_value
#                 half_value = self.flow / 2
#                 updated_lines[reach_line + 1] = f"     {half_value}\n"
        
#         # Write the updated lines back to the file (or you can choose to write to a new file)
#         with open(self.stochICE.flow_file, 'w') as file:
#             file.writelines(updated_lines)
