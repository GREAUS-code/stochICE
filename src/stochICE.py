...
#         print("----------------------------------------------------------")
#         print('---------------Calculating flood area stats---------------')
#         print("----------------------------------------------------------\n")
        
#         # self.stochHECRAS.calculate_flood_areas()
#         # self.stochHECRAS.perform_multivariate_regression()
        
        
#         # self.stop_logging()

#     def __getstate__(self):
#         """Customize pickling by removing unpicklable attributes."""
#         state = self.__dict__.copy()
#         # Remove or replace unpicklable attributes
#         if 'log_file' in state:
#             state['log_file'] = None  # Remove the open file object
#         if 'original_stdout' in state:
#             state['original_stdout'] = None  # Remove the original stdout
#         return state

#     def __setstate__(self, state):
#         """Customize unpickling by restoring necessary attributes."""
#         self.__dict__.update(state)
#         # Restore attributes as needed
#         self.log_file = None  # Log file will need to be reopened
#         self.original_stdout = None
...
#         print("StochICE data directory set up successfully.")
    ...
    #                 values = self.generate_uniform_values(var, params, decimal_places)
    #             elif dist_type == 'GEV':
    #                 values = self.generate_GEV_values(var, params[0], params[1], limit_value=params[2], decimal_places=decimal_places, plot=True)
    #             elif dist_type == 'Gumbel':
    #                 print('doing Gumbel')
    #                 values = self.generate_Gumbel_values(var, params[0], params[1], limit_value=params[2], decimal_places=decimal_places, plot=True)
    #             elif dist_type == 'Normal':
    #                 values = self.generate_normal_values(var,params, decimal_places)
    #             elif dist_type == 'Weibull':
    #                 values = self.generate_Weibull_values(var, params[0], params[1], limit_value=params[2], decimal_places=decimal_places, plot=True)
    #             else:
    #                 raise ValueError(f"Unsupported distribution type: {dist_type}")
    ...
    #     if save_as_pdf:
    #         path = os.path.join(self.distributions_path, f"{var}_distribution.pdf")
    ...
    # def start_logging(self, log_filename="simulation_log.txt"):
    #     """
    #     Starts logging console output to the specified file.
        
    #     Args:
    #         log_filename (str): The name of the file to log console output to.
    #     """
    #     self.log_file = open(os.path.join(self.outputs_path, log_filename), "w")
    #     self.original_stdout = sys.stdout  # Save the original stdout
    #     sys.stdout = self.log_file  # Redirect stdout to the log file
